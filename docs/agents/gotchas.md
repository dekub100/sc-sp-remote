...
41. **`broadcast_spotify_state` and `broadcast_soundcloud_state` are separate** — not a single `broadcast_current_state` like main. Each source broadcasts its own shape.
42. **`handle_client_log`** — SC plugin logs via WebSocket `clientLog` messages (SC can't access DevTools).
43. **`is_queue_full()`** checks `pendingQueueMeta.length` against `maxQueueSize`. Dedup checks `any(m["uri"] == normalized_uri for m in pendingQueueMeta)`.
44. **Extension reconnection** — uses `_connecting` flag to prevent re-entrant `connect()` calls. Max 10 reconnect attempts before giving up.

...
export class ToggleLike extends SingletonAction<ToggleLikeSettings> {
  ...
  private handleMessage = (data: any) => {
    ...
    if (data.source === effectiveSource && typeof data.isLiked === "boolean") {
      const newState = data.isLiked ? 1 : 0;
  ...
  override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<ToggleLikeSettings>): Promise<void> {
  ...
  override async onKeyUp(
    ...
    if (effectiveSource === "soundcloud") {
      wsManager.send({ type: "scPlaybackControl", command: "like" });
    } else {
